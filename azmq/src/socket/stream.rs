...
/// messages shall cause an
/// `Err(`[`ZmqError::HostUnreachable`](crate::ZmqError::HostUnreachable)`)` or
/// `Err(`[`ZmqError::Again`](crate::ZmqError::Again)`)` error.
///
/// To open a connection to a server, use the [`connect()`](method@Socket::connect()) call, and
/// then fetch the socket routing id using [`routing_id()`](method@Socket::routing_id())
/// option.
///
...
///
/// You must send one routing id frame followed by one data frame.
/// The [`SendFlags::SEND_MORE`](const@super::SendFlags::SEND_MORE) flag is required for routing
/// id frames but is ignored on data frames.
impl Socket<Stream> {
